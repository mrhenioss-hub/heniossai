  ...
  return (
    ...
    >
      ...
      <div class="flex flex-col border-b border-border-base shrink-0 bg-background-base">
        ...
        {/* Filter Input */}
        <div class="px-2 pb-2">
          <div class="flex items-center gap-1.5 px-2 py-1 bg-surface-base rounded border border-border-base focus-within:border-primary">
            <Icon name="search" class="w-3.5 h-3.5 text-text-weak shrink-0" />
            <input
              type="text"
              value={filterQuery()}
              ...
              class="w-full bg-transparent text-12-regular text-text-base placeholder:text-text-weak outline-none"
            />
            <Show when={filterQuery()}>
              <button
                ...
                class="text-text-weak hover:text-text-base cursor-pointer"
              >
      ...
      {/* Main Content Area */}
      <Show when={activeDir()} fallback={<ExplorerHomeContent />}>
        <div class="flex-1 min-h-0 overflow-y-auto p-1 text-12-regular" role="tree" aria-label="Project files">
    ...
    <div class="flex flex-col w-full" role="none">
      <ContextMenu>
        <ContextMenu.Trigger
          ...
          class="flex items-center gap-1.5 w-full px-2 py-1 rounded-sm text-left hover:bg-surface-base transition-colors cursor-pointer"
          classList={{
            "bg-surface-raised-base text-text-strong font-medium": isActive(),
            "text-text-base": !isActive(),
          }}
          style={{ "padding-left": `${Math.max(8, props.level * 12 + 8)}px` }}
          onClick={() => {
            if (isDir()) {
        ...
        >
          <Show when={isDir()}>
            <Icon
              ...
              class="w-3.5 h-3.5 shrink-0 text-text-weak"
            />
          </Show>
          <FileIcon node={props.item} expanded={isExpanded()} class="w-4 h-4 shrink-0" />
          <span class="truncate text-12-regular flex-1">{fileName()}</span>
          <Show when={isLoading()}>
            <Icon name="spinner" class="w-3 h-3 shrink-0 animate-spin text-text-weak" />
          </Show>
        </ContextMenu.Trigger>
        <ContextMenu.Portal>
          <ContextMenu.Content class="bg-background-base border border-border-base rounded-md p-1 shadow-lg text-12-regular text-text-base min-w-[160px] z-50">
            <Show when={!isDir()}>
              <ContextMenu.Item
                class="px-2 py-1.5 rounded hover:bg-surface-raised-base cursor-pointer flex items-center gap-2"
                onSelect={() => props.onSelectFile(props.item.path)}
              >
                <Icon name="file" class="w-3.5 h-3.5 text-text-weak" />
            ...
            >
              <Icon name="copy" class="w-3.5 h-3.5 text-text-weak" />
              <span>Copy Path</span>
            </ContextMenu.Item>
            <ContextMenu.Item
              class="px-2 py-1.5 rounded hover:bg-surface-raised-base cursor-pointer flex items-center gap-2"
...
function ExplorerHomeContent() {
  ...
  return (
    <div class="flex h-full flex-col overflow-hidden [container-type:size]">
      <div class="shrink-0">
        <HomeProjectsView
...
function LoadingState(): JSX.Element {
  return (
    <div class="h-full w-full flex flex-col items-center justify-center p-4 text-text-weak gap-2">
      ...
      <span class="text-12-regular">Loading workspace files...</span>
    </div>
  )
}
